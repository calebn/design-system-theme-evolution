...
const PROPOSED_CODE_NAMES: Record<string, string> = {
  ...
  'Text Input (name, two fields)': 'FormRow',
  ...
  'Text Toggle Selector': 'FormField',
...
const CODE_COMPONENT_MAP: CodeComponent[] = [
...

  {
    ...
    tier: 'primitive',
    functionalCategory: 'actions',
    description: 'Flexible icon-only button base. Accepts any icon as children. Only controls visual shape (default = transparent, floating = circular white with shadow). Fixed-icon use cases (close, play, expand-collapse, increment, navigate) are separate convenience wrapper compositions that compose this primitive.',
    htmlElement: 'button',
    figmaSources: [
      'Floating Action Button',
    ],
    props: [
      { name: 'variant', type: "'default' | 'floating'", figmaAxis: 'Style', default: 'default', description: 'Controls visual shape only. default = transparent square, floating = circular white with shadow.' },
      { name: 'scale', type: "'sm' | 'md' | 'lg'", figmaAxis: 'Size', default: 'md' },
    ],
    justification: 'Visual shape primitive for icon-only buttons; accepts any icon as children. Wrapper compositions (CloseButton, ExpandCollapseButton, etc.) compose this for fixed-icon use cases.',
  },
...

  {
    ...
    tier: 'primitive',
    functionalCategory: 'selection',
    description: 'Numbered or solid-style page selector with previous/next buttons. Solid variant includes a play/pause toggle for auto-advancing carousels.',
    htmlElement: 'nav',
    figmaSources: ['Next-Previous Selector'],
    props: [
      { name: 'variant', type: "'numbered' | 'solid'", figmaAxis: 'Style', default: 'numbered' },
      { name: 'scale', type: "'sm' | 'md'", figmaAxis: 'Size', default: 'md' },
      { name: 'state', type: "'default' | 'hover' | 'focus' | 'disabled' | 'play'", figmaAxis: 'State', default: 'default', description: 'play = pause/play toggle active (solid variant only)' },
    ],
  ...
  {
    name: 'FormField',
    directoryName: 'form-field',
    tier: 'composition',
    functionalCategory: 'data-entry',
    description: 'Generic field wrapper providing label, required indicator, description, and error message for any child form control.',
    htmlElement: 'div',
    figmaSources: ['Text Toggle Selector'],
    props: [
      { name: 'label', type: 'string', figmaAxis: '', default: '' },
      { name: 'required', type: 'boolean', figmaAxis: '', default: 'false' },
      { name: 'error', type: 'string', figmaAxis: '', default: '' },
      { name: 'description', type: 'string', figmaAxis: '', default: '' },
    ],
    justification: 'Generic label + error wrapper for any child control (Chakra FormControl / shadcn FormField pattern); Text Toggle Selector frames are usage examples',
  },

  {
    name: 'FormRow',
    directoryName: 'form-row',
    tier: 'composition',
    functionalCategory: 'data-entry',
    description: 'Multi-column row of form fields. Starts with 2-column layout (first/last name pattern); extensible to more columns when additional Figma patterns exist.',
    htmlElement: 'div',
    figmaSources: ['Text Input (name, two fields)'],
    props: [
      { name: 'columns', type: "'2'", figmaAxis: '', default: '2', description: 'Number of equal-width columns. Only 2-column is defined from Figma; extend when more patterns emerge.', proposed: true },
    ],
    justification: 'Multi-column form field row; 2-column is the only Figma-evidenced layout — extensible to 3-4 columns when more patterns emerge',
  },
  ...
  {
    name: 'CloseButton',
    directoryName: 'close-button',
    tier: 'composition',
    functionalCategory: 'actions',
    description: 'Convenience wrapper composing IconButton with ClearIcon pre-wired. Used to dismiss overlays, modals, and toasts. Deferred — not planned for initial implementation.',
    htmlElement: 'button',
    figmaSources: ['Close Button'],
    props: [
      { name: 'scale', type: "'sm' | 'md' | 'lg'", figmaAxis: 'Size', default: 'md' },
    ],
    justification: 'Fixed-icon IconButton wrapper; always renders ClearIcon — separate from IconButton which accepts any icon as children',
  },

  {
    name: 'PlayButton',
    directoryName: 'play-button',
    tier: 'composition',
    functionalCategory: 'actions',
    description: 'Convenience wrapper composing IconButton with PlayIcon pre-wired. Used for media playback controls. Deferred — not planned for initial implementation.',
    htmlElement: 'button',
    figmaSources: ['Play Button'],
    props: [
      { name: 'scale', type: "'sm' | 'md' | 'lg'", figmaAxis: 'Size', default: 'md' },
    ],
    justification: 'Fixed-icon IconButton wrapper; always renders PlayIcon — separate from IconButton which accepts any icon as children',
  },

  {
    name: 'ExpandCollapseButton',
    directoryName: 'expand-collapse-button',
    tier: 'composition',
    functionalCategory: 'actions',
    description: 'Convenience wrapper composing IconButton with a chevron icon. Manages aria-expanded state and icon rotation. Used by Accordion. Deferred — not planned for initial implementation.',
    htmlElement: 'button',
    figmaSources: ['Expand-Collapse Button'],
    props: [
      { name: 'expanded', type: 'boolean', figmaAxis: 'State', default: 'false', description: 'Controls aria-expanded and chevron rotation direction.' },
      { name: 'scale', type: "'sm' | 'md' | 'lg'", figmaAxis: 'Size', default: 'md' },
    ],
    justification: 'Fixed-icon IconButton wrapper with aria-expanded behavior; icon rotation and ARIA semantics require component-level logic not present in base IconButton',
  },

  {
    name: 'IncrementButton',
    directoryName: 'increment-button',
    tier: 'composition',
    functionalCategory: 'actions',
    description: 'Convenience wrapper composing IconButton with IncreaseIcon or DecreaseIcon pre-wired. Used inside QuantityInput. Deferred — not planned for initial implementation.',
    htmlElement: 'button',
    figmaSources: ['Increase-Decrease Buttons'],
    props: [
      { name: 'action', type: "'increment' | 'decrement'", figmaAxis: 'Type', default: 'increment' },
      { name: 'scale', type: "'sm' | 'md' | 'lg'", figmaAxis: 'Size', default: 'md' },
    ],
    justification: 'Fixed-icon IconButton wrapper; action prop selects IncreaseIcon or DecreaseIcon — used inside QuantityInput composition',
  },
...
const FIGMA_FRAME_DESCRIPTIONS: Record<string, string> = {
  ...
  'Next-Previous Selector': 'Prev/next arrow controls with optional page counter ("1/4"). 2 styles: Solid (circular icon buttons + optional play/pause toggle), Numbered (text-based counter + arrows). 2 sizes. 5 states: Default, Hover, Focus, Disabled, Play. Updated to 18 variants (from 4).',
